...
var getTokenFromDB = async function (userId, userType, token) {
  ...
  switch (userType) {
    ...
    case Config.APP_CONSTANTS.DATABASE.USER_ROLES.MANAGER:
    ...
    case Config.APP_CONSTANTS.DATABASE.USER_ROLES.MERCHANT:
      result = await Services.MerchantService.getMerchantPromise(criteria, {}, {});
      if (result && result.length > 0) {
        result[0].type = userType;
        return result[0];
      } else {
      ...
      function (cb) {
        switch (userType) {
          ...
          case Config.APP_CONSTANTS.DATABASE.USER_ROLES.ADMIN:
            ...
            break;
          case Config.APP_CONSTANTS.DATABASE.USER_ROLES.MERCHANT:
            Services.MerchantService.updateMerchant(
              criteria,
              setQuery,
              { new: true },
              function (err, dataAry) {
